...
//  Using data factory to get previous medical information from the database
    $scope.dataFactory.retrieveMedical().then(function() {
        ...
        if($scope.infection == true) {
            $scope.client_shortTermConditions.push('Infection');
        }
        if($scope.inflammation == true) {
            $scope.client_shortTermConditions.push('Inflammation');
        }
        if($scope.flu == true) {
            $scope.client_shortTermConditions.push('Flu');
        }
        if($scope.fever == true) {
            $scope.client_shortTermConditions.push('Fever');
        }
        if($scope.cold == true) {
            $scope.client_shortTermConditions.push('Cold');
        }
        $scope.symptoms = $scope.client_shortTermConditions;
    });
